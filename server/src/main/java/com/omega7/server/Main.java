...
public final class Main {
    ...
    private static void requireProductionSecrets() {
        ...
        if (AUTH_SECRET.length < 32 || BOOTSTRAP_SECRET.length() < 32) {
            throw new IllegalStateException("Sekrety Ω7 muszą mieć co najmniej 32 znaki.");
        }
    }

    private static void requireSecret(String name, String value) {
        if (value == null || value.isBlank()) throw new IllegalStateException("Brak wymaganego sekretu środowiskowego: " + name);
    }
    ...
    private static void health(HttpExchange x) throws IOException {
        ...
        try (Connection c = db(); Statement s = c.createStatement(); ResultSet r = s.executeQuery("SELECT 1")) {
            json(x, 200, obj().put("status", "ok"));
    ...
    private static void bootstrap(HttpExchange x) throws IOException {
        ...
        String g = text(b, "groupId", 128); int d = integer(b, "deviceId", 1, 127);
        require(g != null, "groupId");
        try (Connection c = db()) {
            ...
            byte[] token = random(32);
            insertDevice(c, g, d, b, token);
            insertPreKeys(c, g, d, b);
            c.commit();
            json(x, 201, obj().put("deviceId", d).put("authToken", tokenString(token)));
    ...
    private static void createInvite(HttpExchange x) throws IOException {
        ...
        JsonNode b = readJson(x); String group = text(b, "groupId", 128); int owner = integer(b, "ownerDeviceId", 1, 127);
        require(group != null, "groupId");
        try (Connection c = db()) {
            ...
            json(x, 201, obj().put("inviteId", id.toString()).put("expiresInSeconds", 300).put("inviteToken", tokenString(token)));
            java.util.Arrays.fill(token, (byte) 0);
    ...
    private static void register(HttpExchange x) throws IOException {
        ...
        String group = text(body, "groupId", 128); int device = integer(body, "deviceId", 1, 127); String inviteToken = text(body, "inviteToken", 512);
        require(group != null && inviteToken != null, "groupId/inviteToken");
        try (Connection c = db()) {
            c.setAutoCommit(false);
            UUID inviteId = consumeInvite(c, group, inviteToken);
            if (inviteId == null) { c.rollback(); json(x, 403, error("Nieprawidłowe lub wygasłe zaproszenie.")); return; }
            ...
            byte[] token = random(32);
            insertDevice(c, group, device, body, token);
            insertPreKeys(c, group, device, body);
            c.commit();
            json(x, 201, obj().put("deviceId", device).put("authToken", tokenString(token)));
            java.util.Arrays.fill(token, (byte) 0);
    ...
    private static boolean hasCapacity(Connection c, String group) throws SQLException {
        try (PreparedStatement count = c.prepareStatement("SELECT count(*) FROM devices WHERE group_id=? AND active=true")) {
            count.setString(1, group);
            try (ResultSet rs = count.executeQuery()) { rs.next(); return rs.getInt(1) < MAX_DEVICES; }
        }
    }
        ...
        if (list.isArray() && list.size() > 0) {
            try (PreparedStatement p = c.prepareStatement("INSERT INTO one_time_prekeys(group_id,device_id,prekey_id,prekey) VALUES(?,?,?,?) ON CONFLICT DO NOTHING")) {
                for (JsonNode item : list) {
                    int id = integer(item, "id", 1, Integer.MAX_VALUE); byte[] key = b64(item, "key");
                    if (key.length == 0 || key.length > 4096) throw new IllegalArgumentException("Nieprawidłowy prekey.");
                    p.setString(1, group); p.setInt(2, device); p.setInt(3, id); p.setBytes(4, key); p.addBatch();
                }
                p.executeBatch();
            }
        } else if (b.hasNonNull("preKeyId") && b.hasNonNull("preKey")) {
            int id = integer(b, "preKeyId", 1, Integer.MAX_VALUE); byte[] key = b64(b, "preKey");
            if (key.length == 0 || key.length > 4096) throw new IllegalArgumentException("Nieprawidłowy prekey.");
            try (PreparedStatement p = c.prepareStatement("INSERT INTO one_time_prekeys(group_id,device_id,prekey_id,prekey) VALUES(?,?,?,?) ON CONFLICT DO NOTHING")) {
                p.setString(1, group); p.setInt(2, device); p.setInt(3, id); p.setBytes(4, key); p.executeUpdate();
            }
        }
    }
    ...
    private static void uploadPreKeys(HttpExchange x) throws IOException {
        ...
        JsonNode b = readJson(x); String group = text(b, "groupId", 128); int device = integer(b, "deviceId", 1, 127);
        JsonNode list = b.path("preKeys"); require(group != null && list.isArray(), "groupId/preKeys");
        if (list.size() < 1 || list.size() > MAX_PREKEYS_PER_UPLOAD) { json(x, 400, error("Nieprawidłowa liczba prekeys.")); return; }
        try (Connection c = db()) {
            if (!authorized(c, group, device, token)) { json(x, 403, error("Brak uprawnień.")); return; }
            insertPreKeys(c, group, device, b);
            json(x, 202, obj().put("accepted", true).put("count", list.size()));
        } catch (Exception e) { json(x, 400, error("Nie można zapisać prekeys.")); }
    }

    private static UUID consumeInvite(Connection c, String group, String token) throws Exception {
        byte[] hash = hmac(token.getBytes(StandardCharsets.UTF_8));
        try (PreparedStatement p = c.prepareStatement("UPDATE invites SET consumed_at=now() WHERE group_id=? AND token_hash=? AND consumed_at IS NULL AND expires_at>now() RETURNING invite_id")) {
            p.setString(1, group); p.setBytes(2, hash); try (ResultSet r = p.executeQuery()) { return r.next() ? UUID.fromString(r.getString(1)) : null; }
        }
    }
    ...
    private static void revoke(HttpExchange x) throws IOException {
        if (!method(x, "POST")) return;
        String token = auth(x); if (token == null) { json(x, 401, error("Brak uwierzytelnienia.")); return; }
        JsonNode b = readJson(x); String group = text(b, "groupId", 128); int owner = integer(b, "ownerDeviceId", 1, 127); int target = integer(b, "targetDeviceId", 1, 127);
        require(group != null, "groupId"); if (owner == target) { json(x, 400, error("Właściciel nie może odwołać własnego urządzenia.")); return; }
        try (Connection c = db()) {
            if (!authorized(c, group, owner, token)) { json(x, 403, error("Brak uprawnień.")); return; }
            try (PreparedStatement p = c.prepareStatement("UPDATE devices SET active=false,updated_at=now() WHERE group_id=? AND device_id=? AND active=true")) {
                p.setString(1, group); p.setInt(2, target); if (p.executeUpdate() == 0) { json(x, 404, error("Urządzenie nie istnieje lub jest już odwołane.")); return; }
            }
        ...
        try (Connection c = db()) {
            ...
            try (PreparedStatement q = c.prepareStatement("SELECT registration_id,identity_key,signed_prekey_id,signed_prekey,signed_prekey_signature,kyber_prekey_id,kyber_prekey,kyber_prekey_signature FROM devices WHERE group_id=? AND device_id=? AND active=true")) {
                ...
                try (ResultSet r = q.executeQuery()) {
                    ...
                    ObjectNode out = obj().put("deviceId", device).put("registrationId", r.getInt(1)).put("identityKey", b64(r.getBytes(2)))
                        .put("signedPreKeyId", r.getInt(3)).put("signedPreKey", b64(r.getBytes(4))).put("signedPreKeySignature", b64(r.getBytes(5)))
                        .put("kyberPreKeyId", r.getInt(6)).put("kyberPreKey", b64(r.getBytes(7))).put("kyberPreKeySignature", b64(r.getBytes(8)));
                    try (PreparedStatement pre = c.prepareStatement("SELECT prekey_id,prekey FROM one_time_prekeys WHERE group_id=? AND device_id=? AND consumed_at IS NULL ORDER BY prekey_id LIMIT 1 FOR UPDATE SKIP LOCKED")) {
                        ...
                        try (ResultSet pr = pre.executeQuery()) {
                            if (pr.next()) {
                                int id = pr.getInt(1); byte[] key = pr.getBytes(2);
                                out.put("preKeyId", id).put("preKey", b64(key));
                                try (PreparedStatement consume = c.prepareStatement("UPDATE one_time_prekeys SET consumed_at=now() WHERE group_id=? AND device_id=? AND prekey_id=? AND consumed_at IS NULL")) {
                                    consume.setString(1, group); consume.setInt(2, device); consume.setInt(3, id); consume.executeUpdate();
                                }
    ...
    private static void messages(HttpExchange x) throws IOException {
        if (!method(x, "POST")) return;
        String token = auth(x); if (token == null) { json(x, 401, error("Brak uwierzytelnienia.")); return; }
        JsonNode b = readJson(x); String group = text(b, "groupId", 128), idem = text(b, "idempotencyKey", 128);
        int sender = integer(b, "senderDeviceId", 1, 127), recipient = integer(b, "recipientDeviceId", 1, 127);
        require(group != null && idem != null, "groupId/idempotencyKey");
        byte[] cipher = b64(b, "ciphertext"); if (cipher.length > 256 * 1024) { json(x, 413, error("Koperta jest zbyt duża.")); return; }
        if (sender == recipient) { json(x, 400, error("Niedozwolony adres docelowy.")); return; }
        try (Connection c = db()) {
            if (!authorized(c, group, sender, token)) { json(x, 403, error("Brak uprawnień.")); return; }
            try (PreparedStatement target = c.prepareStatement("SELECT 1 FROM devices WHERE group_id=? AND device_id=? AND active=true")) {
                target.setString(1, group); target.setInt(2, recipient);
                try (ResultSet rs = target.executeQuery()) { if (!rs.next()) { json(x, 404, error("Urządzenie docelowe nie istnieje lub jest odwołane.")); return; } }
            }
            try (PreparedStatement p = c.prepareStatement("INSERT INTO messages(group_id,recipient_device_id,sender_device_id,idempotency_key,ciphertext) VALUES(?,?,?,?,?) ON CONFLICT(group_id,recipient_device_id,idempotency_key) DO NOTHING")) {
                p.setString(1, group); p.setInt(2, recipient); p.setInt(3, sender); p.setString(4, idem); p.setBytes(5, cipher); p.executeUpdate();
            }
    ...
    private static void sync(HttpExchange x) throws IOException {
        if (!method(x, "GET")) return;
        String token = auth(x); if (token == null) { json(x, 401, error("Brak uwierzytelnienia.")); return; }
        String group = q(x, "groupId"), cur = q(x, "cursor"), deviceRaw = q(x, "deviceId");
        if (group == null || group.length() > 128 || deviceRaw == null) { json(x, 400, error("Brak parametrów synchronizacji.")); return; }
        ...
        try (Connection c = db()) {
            ...
            try (PreparedStatement p = c.prepareStatement("SELECT seq,sender_device_id,idempotency_key,ciphertext,created_at FROM messages WHERE group_id=? AND recipient_device_id=? AND seq>? ORDER BY seq ASC LIMIT 100")) {
                ...
                try (ResultSet r = p.executeQuery()) {
                    ...
                    while (r.next()) { next = r.getLong(1); a.add(obj().put("seq", next).put("senderDeviceId", r.getInt(2)).put("idempotencyKey", r.getString(3)).put("ciphertext", b64(r.getBytes(4))).put("createdAt", r.getTimestamp(5).toInstant().toString())); }
                    json(x, 200, obj().put("cursor", next).set("messages", a));
                }
            }
    ...
    private static boolean authorized(Connection c, String group, int device, String token) throws Exception {
        byte[] decoded;
        try { decoded = Base64.getUrlDecoder().decode(token); } catch (IllegalArgumentException e) { return false; }
        if (decoded.length != 32) return false;
        try (PreparedStatement p = c.prepareStatement("SELECT 1 FROM devices WHERE group_id=? AND device_id=? AND active=true AND auth_token_hash=?")) {
            p.setString(1, group); p.setInt(2, device); p.setBytes(3, hmac(decoded));
