...
public final class Main {
    ...
    private static void requireSecret(String name, String value) {
        if (value == null || value.isBlank()) {
            throw new IllegalStateException("Brak wymaganego sekretu środowiskowego: " + name);
        }
    }
    ...
    private static void init(Connection c) throws Exception {
        try (InputStream in = Main.class.getResourceAsStream("/schema.sql")) {
            ...
            for (String statement : sql.split(";")) {
                if (!statement.isBlank()) try (Statement s = c.createStatement()) { s.execute(statement); }
            }
        }
    }
    ...
    private static void bootstrap(HttpExchange x) throws IOException {
        ...
        try(Connection c=db()){
            ...
            byte[] token=random(32); insertDevice(c,g,d,b,token); c.commit();
    ...
    private static void createInvite(HttpExchange x) throws IOException {
        if(!method(x,"POST"))return; String auth=auth(x); if(auth==null){json(x,401,error("Brak uwierzytelnienia."));return;}
        JsonNode b=readJson(x); String group=text(b,"groupId",128); int owner=integer(b,"ownerDeviceId",1,127); require(group!=null,"groupId");
        try(Connection c=db()){if(!authorized(c,group,owner,auth)){json(x,403,error("Brak uprawnień."));return;} byte[] token=random(32); UUID id=UUID.randomUUID();
            try(PreparedStatement p=c.prepareStatement("INSERT INTO invites(invite_id,group_id,owner_device_id,token_hash,expires_at) VALUES(?,?,?,?,now()+interval '5 minutes')")){p.setObject(1,id);p.setString(2,group);p.setInt(3,owner);p.setBytes(4,hmac(token));p.executeUpdate();}
    ...
    private static void register(HttpExchange x) throws IOException {
        if (!method(x, "POST")) return;
        JsonNode body = readJson(x); String group = text(body, "groupId", 128); int device = integer(body, "deviceId", 1, 127); String inviteToken = text(body, "inviteToken", 512);
        require(group != null && inviteToken != null, "groupId/inviteToken");
        try (Connection c = db()) {
            c.setAutoCommit(false); UUID inviteId = consumeInvite(c, group, inviteToken);
            ...
            try (PreparedStatement lock = c.prepareStatement("SELECT group_id FROM omega_groups WHERE group_id=? FOR UPDATE")) {
                lock.setString(1, group); try (ResultSet locked = lock.executeQuery()) { if (!locked.next()) { c.rollback(); json(x,404,error("Grupa nie istnieje.")); return; } }
            }
            try (PreparedStatement count = c.prepareStatement("SELECT count(*) FROM devices WHERE group_id=? AND active=true")) {
                count.setString(1, group); try (ResultSet rs = count.executeQuery()) { rs.next(); if (rs.getInt(1) >= MAX_DEVICES) { c.rollback(); json(x,409,error("Limit 7 urządzeń został osiągnięty.")); return; } }
            }
            byte[] token = random(32); insertDevice(c, group, device, body, token); c.commit();
            json(x, 201, obj().put("deviceId", device).put("authToken", tokenString(token))); java.util.Arrays.fill(token,(byte)0);
    ...
    private static void revoke(HttpExchange x) throws IOException {
        ...
        try (Connection c = db()) {
            ...
            try (PreparedStatement p = c.prepareStatement("UPDATE devices SET active=false WHERE group_id=? AND device_id=? AND active=true")) {
    ...
    private static void keys(HttpExchange x)throws IOException{
        if(!method(x,"GET"))return; String[] parts=x.getRequestURI().getPath().split("/"); if(parts.length<5){json(x,400,error("Brak identyfikatora."));return;}
        ...
        try(Connection c=db();PreparedStatement q=c.prepareStatement("SELECT registration_id,identity_key,signed_prekey_id,signed_prekey,signed_prekey_signature,kyber_prekey_id,kyber_prekey,kyber_prekey_signature FROM devices WHERE group_id=? AND device_id=? AND active=true")){
            q.setString(1,group);q.setInt(2,device);try(ResultSet r=q.executeQuery()){if(!r.next()){json(x,404,error("Urządzenie nie istnieje."));return;}
    ...
    private static void sync(HttpExchange x) throws IOException {
        ...
        String group=q(x,"groupId"), cur=q(x,"cursor"); int device; try{device=Integer.parseInt(q(x,"deviceId"));}catch(Exception e){json(x,400,error("Nieprawidłowe urządzenie."));return;}
        long cursor=cur==null?0:Long.parseLong(cur);
        try(Connection c=db()){
    ...
    private static boolean authorized(Connection c,String group,int device,String token)throws Exception{
        ...
        try(PreparedStatement p=c.prepareStatement("SELECT 1 FROM devices WHERE group_id=? AND device_id=? AND active=true AND auth_token_hash=?")){
            p.setString(1,group);p.setInt(2,device);p.setBytes(3,hmac(decoded));try(ResultSet r=p.executeQuery()){return r.next();}
        }
    }

    private static String auth(HttpExchange x){String h=x.getRequestHeaders().getFirst("Authorization");if(h==null||!h.startsWith("Bearer "))return null;return h.substring(7);}
