...
if(req.method==='GET'&&u.pathname==='/api/health')return json(res,200,{ok:true,version:'3.1.0',service:'jarvis-backend',persistence:process.env.DATABASE_URL?'postgres-configured':'json-fallback'});
...
if(req.method==='POST'&&u.pathname==='/api/push/test'){let sent=0;for(const token of state.push_tokens){if(await sendPush(token,String((await body(req)).title||'JARVIS'),String('Test notification')))sent++}return json(res,200,{sent})}
